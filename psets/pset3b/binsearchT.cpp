...
#endif
template <typename T>
int binarysearch(T *data, const T& key, int lo, int hi) {
	DPRINT(cout << "key=" << key << " lo=" << lo << " hi=" << hi << endl;);
	
	//cout << "your code here \n";
	int mi = lo + (hi - lo) / 2;
	if(key == data[mi]) return mi; //If the key is found in the list, it displays its index in the list.
	else{
		if(hi <= lo) return mi; //If the key is not found, it displays where it is supposed to be appeared if there is one.
		if(key < data[mi]) return binarysearch(data, key, lo, mi);
		if(key > data[mi]) return binarysearch(data, key, mi+1, hi);
	}
	return 0;
}
template <typename T>
// randomly generate a key to search between list[0] and list[size-1].
const T& get_a_key(T *list, int size) {
  	const T& key = rand() % (list[size - 1] + 1 - list[0]) + list[0];
	return key;
}
...
void binarysearch(T *list, int size) {
	...
	const T& key = get_a_key(list, size);
	int idx = binarysearch(list, key, 0, size);
	//cout << "your code here \n";
	if(key == list[idx]) cout << "\t" << key << "\tis     @list[" << idx << "]\n";
	else cout << "\t" << key << "\tis not @list[" << idx << "]\n";
